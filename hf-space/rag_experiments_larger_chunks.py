"""
LARGER CHUNKS Strategy - Target: 66%+ with ±2 pages
===================================================

We've plateaued at 64% with different voting strategies.
The problem is likely RETRIEVAL, not voting.

NEW STRATEGY: Increase chunk size from 3000 to 4000
Larger chunks = more context = better semantic matching
This might help the 3 borderline cases find correct pages.
"""
...
FINAL_TOP_K = 12
BM25_WEIGHT = 0.30
CHUNK_SIZE = 4000  # INCREASED from 3000
CHUNK_OVERLAP = 1200  # Proportional increase
VOTING_EXPONENT = 3.0  # Back to aggressive
...
class LargerChunksRAG:
    ...
    def index_chunks(self, chunks: List[Document]):
        ...
        try:
            self.chroma_client.delete_collection("larger_chunks")
        except:
            pass
...
def run_larger_chunks():
    print("\n" + "="*70)
    print("LARGER CHUNKS Strategy")
    print("="*70)
    print(f"Hypothesis: Bigger chunks = better semantic matching")
    print(f"\nChanges:")
    print(f"  ✓ Chunk size: 3000 → 4000 (+33%)")
    print(f"  ✓ Chunk overlap: 900 → 1200 (30%)")
    print(f"  ✓ Voting: 3.0 (aggressive)")
    ...
    output_path = Path(__file__).parent / "rag_results_larger_chunks.json"
    ...
    print("COMPARISON:")
    print("="*70)
    print(f"Ultimate RAG (3000):  64% (±2), 70% (±5)")
    print(f"Larger Chunks (4000): {accuracy_within_2:.1f}% (±2), {accuracy_within_5:.1f}% (±5)")
