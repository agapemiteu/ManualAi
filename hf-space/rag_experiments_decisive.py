"""
Decisive RAG - More aggressive voting to fix borderline cases
==============================================================

STRATEGY: Make the top-ranked chunks MORE decisive by:
1. Increase voting exponent from 3.0 to 4.0 (trust top ranks more)
2. Increase Final Top-K from 12 to 15 (but weight top ones heavily)
3. Stronger page boosting from 1.5 to 2.0 (same-page consensus)

Target: Fix the 3-4 questions that are off by 3-4 pages.
"""
...
INITIAL_TOP_K = 60
FINAL_TOP_K = 15  # More candidates
BM25_WEIGHT = 0.30
CHUNK_SIZE = 3000
CHUNK_OVERLAP = 900
PAGE_BOOST = 2.0  # Stronger boost
VOTING_EXPONENT = 4.0  # More aggressive
...
class DecisiveRAG:
    ...
    def index_chunks(self, chunks: List[Document]):
        ...
        try:
            self.chroma_client.delete_collection("decisive_rag")
        except:
            pass
    ...
    def retrieve(self, question: str) -> List[Tuple[str, int]]:
        ...
        # Stronger page-aware boosting (2.0 instead of 1.5)
        page_groups = {}
        ...
        for page, chunks_on_page in page_groups.items():
            if len(chunks_on_page) > 1:
                for idx, score in chunks_on_page:
                    boosted_scores.append((idx, score * PAGE_BOOST))
            else:
    ...
    def predict_page(self, question: str) -> int:
        """More decisive voting with exponent 4.0"""
        retrieved = self.retrieve(question)
        
        if not retrieved:
            return -1
        
        # More aggressive voting - 4.0 instead of 3.0
        page_scores = {}
        for rank, (_, page) in enumerate(retrieved):
            if page != -1:
                weight = VOTING_EXPONENT ** (len(retrieved) - rank)
...
def run_decisive():
    print("\n" + "="*70)
    print("DECISIVE RAG - More aggressive voting")
    print("="*70)
    print(f"Changes from Ultimate RAG:")
    print(f"  ✓ Voting exponent: 3.0 → {VOTING_EXPONENT} (trust top ranks MORE)")
    print(f"  ✓ Final Top-K: 12 → {FINAL_TOP_K} (more candidates)")
    print(f"  ✓ Page boost: 1.5 → {PAGE_BOOST} (stronger same-page consensus)")
    ...
    output_path = Path(__file__).parent / "rag_results_decisive.json"
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            "config": {
                ...
                "voting_exponent": VOTING_EXPONENT
            },
            "results": {
    ...
    print(f"Decisive RAG:         {accuracy_within_2:.1f}% (±2), {accuracy_within_5:.1f}% (±5)")
