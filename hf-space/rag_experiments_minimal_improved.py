"""
Minimal Improvement on Ultimate RAG - Target: 68%+ with ±2 pages
================================================================

STRATEGY: Start from our best config (64% ±2, 70% ±5) and make
minimal targeted improvements:

1. Slightly increase Final Top-K from 10 to 12 (more voting candidates)
2. Reduce voting aggressiveness from 3.0 to 2.5 (be more generous)
3. Increase chunk overlap from 900 to 1000 (better continuity)

These small changes should push us from 64% to 68-70% on ±2 pages.
"""
...
# Configuration - MINIMAL TWEAKS from Ultimate RAG
...
# Tweaked from Ultimate RAG
INITIAL_TOP_K = 60
FINAL_TOP_K = 12  # Increased from 10 to 12
BM25_WEIGHT = 0.30
CHUNK_SIZE = 3000
CHUNK_OVERLAP = 1000  # Increased from 900 to 1000

# Copy EnhancedBM25 from ultimate
class EnhancedBM25:
...
class MinimalImprovedRAG:
    ...
    def index_chunks(self, chunks: List[Document]):
        ...
        try:
            self.chroma_client.delete_collection("minimal_improved")
        except:
            pass
        
        self.collection = self.chroma_client.create_collection(
            name="minimal_improved",
    ...
    def predict_page(self, question: str) -> int:
        """TWEAKED: Reduced aggressiveness from 3.0 to 2.5"""
        retrieved = self.retrieve(question)
        
        if not retrieved:
            return -1
        
        # Less aggressive voting - changed from 3.0 to 2.5
        page_scores = {}
        for rank, (_, page) in enumerate(retrieved):
            if page != -1:
                weight = 2.5 ** (len(retrieved) - rank)  # CHANGED FROM 3.0
...
def run_minimal_improved():
    print("\n" + "="*70)
    print("MINIMAL IMPROVEMENT on Ultimate RAG")
    print("="*70)
    print(f"Changes from Ultimate RAG (64% ±2, 70% ±5):")
    print(f"  ✓ Final Top-K: 10 → 12 (more voting candidates)")
    print(f"  ✓ Voting weight: 3.0 → 2.5 (less aggressive)")
    print(f"  ✓ Chunk overlap: 900 → 1000 (better continuity)")
    print(f"\nConfig: Chunk {CHUNK_SIZE}/{CHUNK_OVERLAP}, Top-K {INITIAL_TOP_K}/{FINAL_TOP_K}")
    print("="*70 + "\n")
    ...
    output_path = Path(__file__).parent / "rag_results_minimal_improved.json"
    ...
    print(f"Minimal Improved:     {accuracy_within_2:.1f}% (±2), {accuracy_within_5:.1f}% (±5)")
    
    if accuracy_within_2 > 64:
        improvement = accuracy_within_2 - 64
        print(f"\n✅ IMPROVEMENT: +{improvement:.1f} percentage points on ±2 pages!")
    elif accuracy_within_2 >= 64:
        print(f"\n✓ Maintained performance at {accuracy_within_2:.1f}%")
    else:
        decline = 64 - accuracy_within_2
        print(f"\n⚠️  Slight decline: -{decline:.1f} percentage points")
    
    print("="*70)

if __name__ == "__main__":
    try:
        run_minimal_improved()
